/// Centripetal Catmull-Rom spline, closed-loop only.
/// Ported from src/catmull-rom.js.
...
pub fn tangent_at_point(points: &[Pt], i: usize) -> Pt {
    let n = points.len();
    if n < 3 { return Vector3::new(1.0, 0.0, 0.0); }
    ...
    if len > 1e-9 { t /= len; } else { t = Vector3::new(1.0, 0.0, 0.0); }
    t
}
