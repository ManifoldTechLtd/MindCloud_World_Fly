//! Flight HUD — fighter-jet style OSD. Ported from native `src/hud.rs` (egui custom paint), with
//! the gates/race panel dropped (gates aren't ported yet) and egui 0.33 API adaptations
//! (`Frame::NONE`, `corner_radius`, `Margin::same`). Pure egui drawing; the Bevy glue lives in
//! `flight::hud_system`.

use bevy_egui::egui::{self, Color32, FontId, Pos2, Rect, RichText, Stroke, Vec2};
        ...
        .show(ctx, |ui| {
            ...
            egui::Frame::NONE
                ...
                .show(ui, |ui| {
                    ...
                    let mode = match drone.flight_mode {
                        FlightMode::Fpv => "FPV",
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.label(
                            RichText::new(if armed { "ARMED" } else { "DISARMED" })
                                .color(arm_c)
                                .size(12.0)
                                .strong(),
                        );
...
fn draw_horizon(p: &egui::Painter, cx: f32, cy: f32, w: f32, h: f32, pitch: f32, roll: f32) {
    ...
    let rotate = |dx: f32, dy: f32| -> Pos2 {
        Pos2::new(cx + dx * cos_r - dy * sin_r, cy + dx * sin_r + dy * cos_r)
    };

    // Horizon line left + right
...
fn draw_heading(p: &egui::Painter, w: f32, h: f32, cx: f32, cy: f32, yaw: f32) {
    ...
    for d in (-180..=540).step_by(5) {
        ...
        if x < cx - bar_w / 2.0 - 5.0 || x > cx + bar_w / 2.0 + 5.0 {
            continue;
        }
