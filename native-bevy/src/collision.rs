//! Octree-based collision detection for point-cloud (gaussian-splat) scenes. Ported from
//! `native/src/collision.rs` (itself a port of `src/collision.js`): build a spatial index over the
//! splat center positions, then sphere-vs-cloud query + inverse-distance-weighted surface normal so
//! the drone can be pushed out of geometry and bounced. Pure data structure — no Bevy / Drone deps;
//! `flight::check_collision` ties it to the drone, matching native's `check_collision`.
...
impl OctreeNode {
    ...
    /// True if the node AABB intersects the query sphere (closest-point-on-AABB test).
...
pub struct SphereHit {
    /// Cloud point index. Unused by the collision response (which only needs position + distance),
    /// but kept to mirror native's API for the deferred gate-clearance query (Phase 7).
    #[allow(dead_code)]
    pub index: u32,
    pub x: f32,
    pub y: f32,
...
/// Compute collision normal + penetration from sphere query results: the surface normal is the
/// inverse-distance-weighted average of (drone - point) directions (closer points dominate), and the
/// penetration is `radius - nearest_point_distance`. `None` when there are no hits.
pub fn compute_collision_response(
    ...
    if len < 0.0001 {
        // Degenerate (drone centred in a symmetric shell): push straight up.
